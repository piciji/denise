...
auto Denise::strlong() -> void {
    // todo: for long lines (NTSC) only.
    // NTSC has 227.5 CCK scan line length. every other line has 1 lores pixel offset.
    // Denise buffers some generated pixel and use delayed pixel to compensate this offset.
    // not really needed to emulate this, because output goes to a frame buffer and we don't need to produce HSYNC pulses
    // to ensure alignment in this frame buffer.
}
...
auto Denise::setDiwHigh(uint16_t value) -> void {
    hStart &= 0xff;
    hStart |= ((value >> 5) & 1) << 8;
    hStop &= 0xff;
    hStop |= ((value >> 13) & 1) << 8;
...
auto Denise::killEHB(bool state) -> void {
    if (state) {
        for(int i = 0; i < 32; i++)
            colors[32 + i] = colors[i];
    } else {
        for (int i = 0; i < 32; i++)
            colors[32 + i] = ehbCols[colors[i] & 0xfff];
    }
}
...
auto Denise::setBplCon0( uint16_t value ) -> void {
  ...
  //  if ((value ^ bplCon0) & (0x8000 | 0x40))
    //    agnus.interface->log((value & 0x40) ? "shres" : ((value & 0x8000) ? "hires" : "lores"));

    if (value & 0x40) {
        if (frameMode == LORES_FRAME) {
            agnus.doubleResMidframe<true>(false);
            frameMode = SHRES_FRAME;
        } else if (frameMode == HIRES_FRAME) {
            agnus.doubleResMidframe<false>(true);
            frameMode = SHRES_FRAME;
        }

    } else if (value & 0x8000) {
        if (frameMode == LORES_FRAME) {
            agnus.doubleResMidframe<false>(false);
            frameMode = HIRES_FRAME;
        }
    }
...
auto Denise::setBplCon3(uint16_t value) -> void {
    if (value == bplCon3)
        return;
...
auto Denise::csync(bool state) -> void {
    if (!extblanken)
        return;
...
auto Denise::updateECSFeatures() -> void {
    brdrblnk = ecsena() && (bplCon3 & 0x20);
    extblanken = ecsena() && (bplCon3 & 1);
}
...
auto Denise::process(int offset) -> void {
    ...
    if (disableSequencer) {
        ...
        switch (bplCon0 & (0x8000 | 0x400 | 0x40)) { // hires, dpf  ... HAM is not relevant
            case 0: process<false, false, false, false, false>(cycles, activePlanes); break;     // lores
            case 0x400: process<false, false, false, true, false>(cycles, activePlanes); break; // dpf

            case 0x8000: process<true, false, false, false, false>(cycles, activePlanes); break;  // hires
            case 0x8000 | 0x400: process<true, false, false, true, false>(cycles, activePlanes); break; // hires dpf

            case 0x40: process<false, true, false, false, false>(cycles, activePlanes); break;     // shres
            case 0x440: process<false, true, false, true, false>(cycles, activePlanes); break; // shres dpf

            case 0x8040: process<false, true, false, false, false>(cycles, activePlanes); break;  // shres hires
            case 0x8040 | 0x400: process<false, true, false, true, false>(cycles, activePlanes); break; // shres hires dpf
        }

    } else {
        switch (bplCon0 & (0x8000 | 0x800 | 0x400 | 0x40)) { // hires, ham, dpf
            case 0: process<false, false, false, false>(cycles, activePlanes); break;     // lores
            case 0x400: process<false, false, false, true>(cycles, activePlanes); break; // dpf

            case 0x8000: process<true, false, false, false>(cycles, activePlanes); break;  // hires
            case 0x800: process<false, false, true, false>(cycles, activePlanes); break; // ham

            case 0x8000 | 0x400: process<true, false, false, true>(cycles, activePlanes); break; // hires dpf
            case 0x8000 | 0x800: process<true, false, true, false>(cycles, activePlanes); break; // hires ham

            case 0x800 | 0x400: process<false, false, true, true>(cycles, activePlanes); break;
            case 0x8000 | 0x800 | 0x400: process<true, false, true, true>(cycles, activePlanes); break;
            // shres
            case 0x40: process<false, true, false, false>(cycles, activePlanes); break;     // shres
            case 0x440: process<false, true, false, true>(cycles, activePlanes); break; // shres dpf

            case 0x8040: process<false, true, false, false>(cycles, activePlanes); break;  // shres hires
            case 0x840: process<false, true, true, false>(cycles, activePlanes); break; // shres ham

            case 0x8040 | 0x400: process<false, true, false, true>(cycles, activePlanes); break; // shres hires dpf
            case 0x8040 | 0x800: process<false, true, true, false>(cycles, activePlanes); break; // shres hires ham

            case 0x840 | 0x400: process<false, true, true, true>(cycles, activePlanes); break;
            case 0x8040 | 0x800 | 0x400: process<false, true, true, true>(cycles, activePlanes); break;
        }
    }
...
template<bool _hires, bool _shres, bool _ham, bool _doublePlayfield, bool _display> inline auto Denise::process(const int cycles, const int _activePlanes) -> void {
    constexpr bool _lores = !_hires && !_shres;
    constexpr int _pix = _shres ? 8 : (_hires ? 4 : 2);    
    constexpr uint16_t _pfMask = _shres ? 3 : (_hires ? 7 : 15);
    ...
    for (int c = 0; c < cycles; c++) {
        ...
        for(int p = 0; p < _pix; p++) {
            ...
            if (_lores || ((p & 1) == 0)) {
                ...
                if (spr0.armed && (hPos == spr0.x) && (!_shres || ((p & 2) == spr0.shsh10)))
                    spr0.shift = (spr0.datA << 16) | spr0.datB;                
                if (spr1.armed && (hPos == spr1.x) && (!_shres || ((p & 2) == spr1.shsh10)))
                    spr1.shift = (spr1.datA << 16) | spr1.datB;
                if (spr2.armed && (hPos == spr2.x) && (!_shres || ((p & 2) == spr2.shsh10)))
                    spr2.shift = (spr2.datA << 16) | spr2.datB;
                if (spr3.armed && (hPos == spr3.x) && (!_shres || ((p & 2) == spr3.shsh10)))
                    spr3.shift = (spr3.datA << 16) | spr3.datB;
                if (spr4.armed && (hPos == spr4.x) && (!_shres || ((p & 2) == spr4.shsh10)))
                    spr4.shift = (spr4.datA << 16) | spr4.datB;
                if (spr5.armed && (hPos == spr5.x) && (!_shres || ((p & 2) == spr5.shsh10)))
                    spr5.shift = (spr5.datA << 16) | spr5.datB;
                if (spr6.armed && (hPos == spr6.x) && (!_shres || ((p & 2) == spr6.shsh10)))
                    spr6.shift = (spr6.datA << 16) | spr6.datB;
                if (spr7.armed && (hPos == spr7.x) && (!_shres || ((p & 2) == spr7.shsh10)))
                    spr7.shift = (spr7.datA << 16) | spr7.datB;
            ...
            if constexpr (_display) {
                ...
                if (!borderFlipFlop && enableDisplay) {
                    ...
                    color = (_hBlank || _brdrblnk) ? 0 : colors[0];

                *(linePtr + linePos++) = color;


                if constexpr (_lores) {
                    if (frameMode & (HIRES_FRAME | SHRES_FRAME)) {
                        if (frameMode == HIRES_FRAME) {
                            *(linePtr + linePos++) = color;
                        } else { // SHRES
                            *(linePtr + linePos++) = color;
                            *(linePtr + linePos++) = color;
                ...
                } else if constexpr (_hires) {
                    if (frameMode == SHRES_FRAME)
                        *(linePtr + linePos++) = color;
                }

                // if Agnus misses sync
                linePos &= _shres ? 0x7ff : 0x3ff;
            } else {
                if (shifterA | shifterB) {
                    shifterA = (shifterA << 1) & ~(0x1000100010000);
                    shifterB = (shifterB << 1) & ~(0x1000100010000);
            ...
            if (_lores || (_shres && ((p & 3) == 3)) || (_hires && (p & 1))) {
                if (pfShift) {
                    if (pfShift & PF1_SHIFT) {
                        if ((hPos & _pfMask) == delayPf1) {
                            pfShift &= ~PF1_SHIFT;
