...
auto Paula::updateModulation() -> void {
    for( uint8_t nr = 0; nr < 4; nr++) {
        ...
        cha.napnav = (!cha.audav && !cha.audap) || cha.audav;
    }
}
...
template<uint8_t nr> auto Paula::stateMainloop() -> void {
    Channel& cha = channels[nr];
...
template<uint8_t nr> auto Paula::percntrld() -> void {
    Channel& cha = channels[nr];
    agnus.updateEvent<Agnus::EVENT_AUDIO_STATE>(~0, cha.perLatch ? cha.perLatch : 0x10000);
    cha.clock = agnus.getEventClock<Agnus::EVENT_AUDIO_STATE>();
    updateAudioEvent();
}
...
auto Paula::updateAudioEvent() -> void {
    ...
    unsigned nextClock = cha0.clock;

    if (!nextClock || (cha1.clock && (cha1.clock < nextClock)))
        nextClock = cha1.clock;

    if (!nextClock || (cha2.clock && (cha2.clock < nextClock)))
        nextClock = cha2.clock;

    if (!nextClock || (cha3.clock && (cha3.clock < nextClock)))
        nextClock = cha3.clock;

    if (!nextClock)
        agnus.setEventInactive<Agnus::EVENT_AUDIO_STATE>();
    else
        agnus.updateEvent<Agnus::EVENT_AUDIO_STATE>(~0, nextClock);
}
