...
struct ExpansionPort {
    ...
    std::function<uint16_t (bool& writeAccess)> listenAddrBus;
    std::function<uint8_t ()> listenDataBus;
    ...
    virtual auto readIo1( uint16_t addr ) -> uint8_t { return vicII->getLastReadedValue(); }
    
    virtual auto readIo2( uint16_t addr ) -> uint8_t { return vicII->getLastReadedValue(); }
    
    virtual auto writeIo1( uint16_t addr, uint8_t data ) -> void {}
    
    virtual auto writeIo2( uint16_t addr, uint8_t data ) -> void {}
    
    virtual auto readRomL( uint16_t addr ) -> uint8_t { return vicII->getLastReadedValue(); }
    
    virtual auto readRomH( uint16_t addr ) -> uint8_t { return vicII->getLastReadedValue(); }
