...
static colHashTable* init_col_hash_table(uint32_t tableSize){
  colHashTable* colhash = (colHashTable*)malloc(sizeof(colHashTable));
  colhash->tableSize = getNextPrimePower2(tableSize); // increase table size to next prime number
  colhash->frequ = (uint32_t*)malloc(sizeof(uint32_t) * colhash->tableSize);
  colhash->hashTable = (uint8_t*)malloc(3 * colhash->tableSize);
  colhash->indexUsed = (uint8_t*)malloc(colhash->tableSize);
  colhash->pPalette = (uint8_t*)malloc(3 * colhash->tableSize);
  colhash->colIdx = (uint32_t*)malloc(sizeof(uint32_t) * colhash->tableSize);
...
static void resize_col_hash_table(colHashTable* colhash){
  ...
  colhash->pPalette = (uint8_t*)realloc(colhash->pPalette, 3 * tableSizeNew);
  colhash->colIdx = (uint32_t*)realloc(colhash->colIdx, sizeof(uint32_t) * tableSizeNew);
  uint8_t* hashTable_new = (uint8_t*)malloc(3 * tableSizeNew);
  uint8_t* indexUsed_new = (uint8_t*)malloc(tableSizeNew);
  uint32_t* frequ_new = (uint32_t*)malloc(sizeof(uint32_t) * tableSizeNew);
  ...
  for(uint32_t j = 0; j < colhash->tableSize; ++j) { // TBD (no improvement when tested): easier to loop over pPalette and also leave pPalette in place?, if indexUsed is also unnecessary then
    if(colhash->indexUsed[j] == 1){
      uint32_t h = col_hash(colhash->hashTable + 3 * j, hashTable_new, indexUsed_new, tableSizeNew, (cgif_chan_fmt)3); // recompute hash with new table size
...
static uint32_t* hash_to_dense(colHashTable* colhash, cgif_chan_fmt fmtChan) {
  uint32_t* frequDense = (uint32_t*)malloc(sizeof(uint32_t) * colhash->cnt);
  uint32_t h;
  (void)fmtChan;
  for(uint32_t i = 0; i < colhash->cnt; ++i) {
    h = col_hash(colhash->pPalette + 3 * i, colhash->hashTable, colhash->indexUsed, colhash->tableSize, (cgif_chan_fmt)3);
...
static uint32_t quantize_and_dither(colHashTable* colhash, const uint8_t* pImageDataRGB, uint32_t numPixel, uint32_t width, cgif_chan_fmt fmtChan, uint8_t* pImageData, uint8_t* pPalette256, uint8_t depthMax, uint8_t dithering, int hasAlpha, uint8_t* pBef, cgif_chan_fmt befFmtChan) {
  ...
  if(colhash->cnt > colMax) { // color-quantization is needed
    ...
    free(pFrequDense);
    float* pImageDataRGBfloat = (float*)malloc(fmtChan * numPixel * sizeof(float)); // TBD fmtChan + only when hasAlpha
...
CGIFrgb* cgif_rgb_newgif(const CGIFrgb_Config* pConfig) {
  ...
  pGIFrgb = (CGIFrgb*)malloc(sizeof(CGIFrgb));
...
cgif_result cgif_rgb_addframe(CGIFrgb* pGIF, const CGIFrgb_FrameConfig* pConfig) {
  ...
  pNewBef = (uint8_t*)malloc(pConfig->fmtChan * MULU16(imageWidth, imageHeight));
  memcpy(pNewBef, pConfig->pImageData, pConfig->fmtChan * MULU16(imageWidth, imageHeight));
  fConfig.pLocalPalette = aPalette;
  fConfig.pImageData = (uint8_t*)malloc(pGIF->config.width * (uint32_t)pGIF->config.height);
  ...
  cgif_result r = (cgif_result)cgif_addframe(pGIF->pGIF, &fConfig);
...
cgif_result cgif_rgb_close(CGIFrgb* pGIF) {
  cgif_result r = (cgif_result)cgif_close(pGIF->pGIF);
  free(pGIF->pBefImageData);
  free(pGIF);
  return r;
}
