...
void main() {
	...
	// Reading the texels
	vec3 ul = texture(source[0], pC4     ).xyz; ul*=ul;
	vec3 ur = texture(source[0], pC4 + dx).xyz; ur*=ur;
	vec3 dl = texture(source[0], pC4 + dy).xyz; dl*=dl;
	vec3 dr = texture(source[0], pC4 + ps).xyz; dr*=dr;
...
// calculating scanlines
	...
	vec3 l3 = texture(source[0], pC4 -x2 ).xyz; l3*=l3;
	vec3 l2 = texture(source[0], pC4 -dx ).xyz; l2*=l2;
	vec3 l1 = texture(source[0], pC4     ).xyz; l1*=l1;
	vec3 r1 = texture(source[0], pC4 +dx ).xyz; r1*=r1;
	vec3 r2 = texture(source[0], pC4 +x2 ).xyz; r2*=r2;
	vec3 r3 = texture(source[0], pC4 +x3 ).xyz; r3*=r3;

	vec3 t1 = (l3*wl3 + l2*wl2 + l1*wl1 + r1*wr1 + r2*wr2 + r3*wr3)*wt;
	
	l3 = texture(source[0], pC4 -x2 -dy).xyz; l3*=l3;
	l2 = texture(source[0], pC4 -dx -dy).xyz; l2*=l2;
	l1 = texture(source[0], pC4     -dy).xyz; l1*=l1;
	r1 = texture(source[0], pC4 +dx -dy).xyz; r1*=r1;
	r2 = texture(source[0], pC4 +x2 -dy).xyz; r2*=r2;
	r3 = texture(source[0], pC4 +x3 -dy).xyz; r3*=r3;
	...
	l3 = texture(source[0], pC4 -x2 +y2).xyz; l3*=l3;
	l2 = texture(source[0], pC4 -dx +y2).xyz; l2*=l2;
	l1 = texture(source[0], pC4     +y2).xyz; l1*=l1;
	r1 = texture(source[0], pC4 +dx +y2).xyz; r1*=r1;
	r2 = texture(source[0], pC4 +x2 +y2).xyz; r2*=r2;
	r3 = texture(source[0], pC4 +x3 +y2).xyz; r3*=r3;
