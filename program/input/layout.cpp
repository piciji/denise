...
auto InputManager::automap( KeyboardLayout::Type type, Emulator::Interface::Key key ) -> std::vector<std::vector<Hid::Key>> {
    ...
    if(GUIKIT::Application::isCocoa()) {
        ...
        switch(type) {
            ...
            case KeyboardLayout::Type::Uk:
                ...
                if ( key == EmuKey::ShiftShared9 ) return {{Hid::Key::D2, Hid::Key::ShiftRight}};
                if ( key == EmuKey::Shared9 ) return {{Hid::Key::D3, Hid::Key::AltRight}};
                if ( key == EmuKey::Shared8 ) return {{Hid::Key::Backslash}};
    ...
    if ( key == EmuKey::SystemRight ) return {{Hid::Key::SuperRight}};    
