...
Debugger::CIA::Chip::Port::Port(uint8_t port) {
...
Debugger::CIA::Chip::PortIO::PortIO(uint8_t chipNr, uint8_t portNr, Debugger* debugger) {
...
Debugger::CIA::Chip::Timer::Timer(uint8_t portNr) {
...
Debugger::CIA::Chip::IntData::IntData() {
    icr.setText( "ICR:" );
    ir.setText( "IR" );
    flag.setText( "Flag" );
    sp.setText( "SP" );
...
Debugger::CIA::Chip::IntMask::IntMask() {
    mask.setText( "Mask:" );
    ir.setText( "IR" );
    flag.setText( "Flag" );
    sp.setText( "SP" );
...
Debugger::CIA::Chip::Tod24bit::Tod24bit() {
    counter.setStore( 0 );
    counter.setText( "0" );
...
Debugger::CIA::Chip::Shifter::Shifter() {
    sdr.setStore( 0 );
    sdr.setText( "0" );
...
Debugger::CIA::Chip::Chip(uint8_t chipNr, Debugger* debugger)
...
Debugger::CIA::CIA( Debugger* debugger )
...
auto Debugger::build() -> void {
    ...
    switch (mode) {
        ...
        case Mode::CIA: _title += " CIA"; break;
        default: break;
    }
...
template<typename T> auto Debugger::updateCia(T& s) -> void {
    for (int i = 0; i < 2; i++) {
        auto& c = cia->chip[i];
        auto& sc = s.cia[i];
