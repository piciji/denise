...
auto OpenGL::refresh(bool disallowShader) -> void {
	...
	for(auto& p : programs) {
		...
		_glUniform4f("outputSize", viewport.width, viewport.height, 1.0 / viewport.width, 1.0 / viewport.height);
	...
	render(viewport.x, viewport.y, viewport.width, viewport.height);
