...
struct D3d9SplashScreen : SplashScreen {
    ...
    auto updateTex(Viewport& _viewport) -> bool {
        ...
        if (!texture || (s == SplashScreen::TEXTURE_UPDATE)) {
            if(texture)
                texture->Release();
            ...
            if (lpD3DDevice->CreateTexture( texStorageWidth, texStorageHeight, 1, 0, D3DFMT_A8R8G8B8,
                                        static_cast<D3DPOOL> (D3DPOOL_MANAGED), &texture, nullptr) != D3D_OK) {
                finish();
                return false;
            }
        }
