...
struct Png {
    ...
    auto load(const uint8_t* sourceData, unsigned sourceSize) -> bool {
        ...
        while( offset < sourceSize ) {
            ...
            if(fourCC == (unsigned)FourCC::IEND) break;
        ...
        bool result = Decode::inflate(interlacedData, interlacedSize, compressedData + 2, compressedSize - 6);
        free(compressedData);
    ...
    auto readbits(const uint8_t*& data) -> unsigned {
    ...
    ~Png() { if(data) delete[] data; }

protected:
    auto interlace(unsigned pass, unsigned index) -> unsigned {
    ...
    auto deinterlace(const uint8_t*& inputData, unsigned pass) -> bool {
        unsigned xd = interlace(pass, 0), yd = interlace(pass, 1);
        unsigned xo = interlace(pass, 2), yo = interlace(pass, 3);
        unsigned width  = (info.width  + (xd - xo - 1)) / xd;
        unsigned height = (info.height + (yd - yo - 1)) / yd;
    ...
    auto filter(uint8_t* outputData, const uint8_t* inputData, unsigned width, unsigned height) -> bool {
    ...
    auto read(const uint8_t* data, unsigned length) -> unsigned {
        unsigned result = 0;
